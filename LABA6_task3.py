...
def to_csv_file(filename, headers, rows, delimiter, new_line):
    f = open(filename, 'w', encoding='utf8')  # открываем файл на перезапись
    count = 0 #завожу счетчик, чтобы отследить элемент в списке
    for i in headers:
        count = count + 1
        f.write(i)
        if count != len(headers):
            f.write(delimiter) #для последнего элемента запятая не ставится

    f.write(new_line) #перенос строки
    count = 0

    for i in rows: # аналогично циклу выше, заводится счетчик для отслеживания последнего элемента
        for j in i:
            count = count + 1
            f.write(j)
            if count != len(i):
